...
from algorithms.maddpg_continous.agents.maddpg.DDPG_agent import DDPG
from algorithms.maddpg_continous.agents.maddpg.buffer import BUFFER
...
class MADDPG():
    ...
    def learn(self, batch_size, gamma):
        ...
        for agent_id, agent in self.agents.items():
            ...
            # Compute the actor loss and regularization (actor_loss_pse)
            action, logits = agent.action(obs[agent_id], model_out=True)
            act[agent_id] = action  # Replace the action in the batch with the one from the actor
            ...
            losses_dict[agent_id] = {
                'critic_loss': critic_loss.item(),
                'actor_loss': actor_loss.item(),
                'actor_loss_pse': actor_loss_pse.item()
            }

        return losses_dict  # Return the losses for each agent
    ...
    def save_model(self, save_path):
        for agent_id in self.dim_info.keys():
            self.agents[agent_id].actor.save_checkpoint(is_target = False, save_dir = save_path)
            self.agents[agent_id].target_actor.save_checkpoint(is_target = True, save_dir = save_path)
            self.agents[agent_id].critic.save_checkpoint(is_target = False, save_dir = save_path)
            self.agents[agent_id].target_critic.save_checkpoint(is_target = True, save_dir = save_path)
